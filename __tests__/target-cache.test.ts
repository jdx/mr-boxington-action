...
describe('target cache pruning', () => {
  ...
  it('omits legacy copied build-script shims but preserves tiny launchers', async () => {
    ...
    const legacy = path.join(target, 'debug', 'build', 'legacy-hash', 'build_script_build-legacy')
    const legacyAlias = path.join(path.dirname(legacy), 'build-script-build')
    const launcher = path.join(target, 'debug', 'build', 'current-hash', 'build_script_build-current')
    const launcherAlias = path.join(path.dirname(launcher), 'build-script-build')
